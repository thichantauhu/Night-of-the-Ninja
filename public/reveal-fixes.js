/* Reveal board: show all played cards in number order, player name, and highlight the active action. */
(function(){
...
.reveal-card-fixed{width:145px;min-height:158px;border:2px solid #d1d5da;border-radius:12px;background:#fff;padding:12px;box-sizing:border-box;display:flex;flex-direction:column;align-items:flex-start;text-align:left;box-shadow:0 3px 10px #00000008;transition:.2s}
...
.reveal-card-fixed .card-desc{font-size:11px;color:#6c7078;line-height:1.35;margin-top:5px}
...
@media(max-width:680px){.reveal-card-fixed{width:calc(50% - 6px);min-width:130px}.reveal-cards-fixed{gap:8px}}
`;
 ...
 return `<div class="reveal-board-fixed">
   ...
   <div class="reveal-cards-fixed">${all.map((x,i)=>{
     ...
     return `<div class="reveal-card-fixed ${cls}">
       ...
       <div class="card-number">${x.card.num??''}</div>
       <div class="card-name">${esc(TYPE[x.card.type]||x.card.name||x.card.type)}</div>
       ${x.card.name&&x.card.name!==(TYPE[x.card.type]||x.card.name)?`<div class="card-desc">${esc(x.card.name)}</div>`:''}
...
/* app.js resolves renderNightCenter through its global function binding, so wrap render itself. */
